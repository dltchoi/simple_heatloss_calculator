def heatloss_estimate (gasuse_total, dhw_monthly, balance_point, furnace_eff, hl_derate, location): #, heatpumpsize, heatpumpcap_17_c1, heatpumpcap_47_c1, heatpumpcap_17_c2, heatpumpcap_47_c2, heatpumpcap_17_c3, heatpumpcap_47_c3, hp_derate, derate_c1, derate_c2, derate_c3):
    ...
    estimate_2021=[]
    start_2021 = '2023-01-01 00:00:00'
    end_2021 = '2023-12-31 23:00:00'
    # import weather data and specify design temperature based on location (GTHA locations)
    if location == 'toronto':
        weatherdata = pd.read_csv("static/7yr_toronto_weather.csv",parse_dates=['date_time_local'])
        ...
        weatherdata = pd.read_csv("static/7yr_caledon_weather.csv",parse_dates=['date_time_local'])
        weatherdata = weatherdata.set_index('date_time_local')
        estimate_2021 = weatherdata.loc[start_2021 : end_2021]
    ...
    #    dtemp = -21
        ...
        weatherdata = pd.read_csv("static/7yr_brampton_weather.csv",parse_dates=['date_time_local'])
        weatherdata = weatherdata.set_index('date_time_local')
        estimate_2021 = weatherdata.loc[start_2021 : end_2021]
        location_name = 'Brampton'
        dtemp = -19
    elif location == 'mississauga':
        weatherdata = pd.read_csv("static/7yr_mississauga_weather.csv",parse_dates=['date_time_local'])
        weatherdata = weatherdata.set_index('date_time_local')
        estimate_2021 = weatherdata.loc[start_2021 : end_2021]
    ...
    # calculate heatloss for 7-year period (2017-2023)
    ...
    for point in temp_points:
        ...
        hour_count = hour_count/7 # average over 7 years
        load_sum = load_sum/7
