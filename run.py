...
def calculator_output():
    ...
    return render_template('calculator_output.html', heatloss_url=heatloss_url, heatdist_url=heatdist_url, dtemp=dtemp, heatloss_design=round(heatloss_design), gasuse_total=int(gasuse_total), dhw_annual=int(dhw_monthly*12), balance_point=int(balance_point), furnace_eff=furnace_eff, gasuse_annual=round(gasuse_annual), location_name=location_name, derate=round(derate))#, size_name=size_name, derate_c1=derate_c1, derate_c2=derate_c2, derate_c3=derate_c3) #,  hp_derate=round(hp_derate),heatpumpsize=heatpumpsize #, switchlist=switchtemp_list, curve1=curve1, curve2=curve2, curve3=curve3, curve1_result=curve1_result, curve2_result=curve2_result, curve3_result=curve3_result, scop=scop, base_cost=round(base_cost), hybrid_costs=hybrid_costs, elec_costs=elec_costs, ziplist=ziplist,)
